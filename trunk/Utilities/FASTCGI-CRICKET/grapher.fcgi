...
use lib "/data2/mon-share/home/cricket/cricket/lib";
...
#    servers => [ { address => '/tmp/wwwrun-memcached126.socket', noreply => 1 } ],
...
while (my ($server, $version) = each %$versions) {
    print STDERR "$server version is $version\n";
}
...
# syc:
$gPollingInterval = 1 * 60;     # defaults to 1 minutes
...
sub doGraph {
    ...
    if (defined($gQ->param('use-gprint'))) {
...
#	$useGtitle=1;
#    }
    if(defined($gQ->param('make-small'))) {
    ...
    foreach $thisName (@mtargets) {
	    ...
	    # Added by syc
            if (defined($gQ->param('use-legend'))) {
